...
/**
 * AppUrlManager handles HTTP request parsing and creation of URLs based on a set of rules.
 *
 * AppUrlManager is configured as an application component in [[\yii\base\Application]] by default.
 ...
 * @since 2.0
 */
class ComUrlManager extends Component
{
    /**
    ...
    /**
     * 判断是否有 $this->routeParam 参数 并且脚本为index.php
     * @author caoxl
     * @param Request $request request组件
     * @return boolean
     */
    public function hasRouteParam($request)
    {
        ...
        switch($method)
        {
            case 'POST':
            case 'PUT':
                $params = array_merge($params, $request->getBodyParams());
                break;
            default:
                break;
        }
        if(array_key_exists($this->routeParam, $params) && $params[$this->routeParam] && preg_match('/^(\/index\.php|\\index\.php)/isU', $scriptUrl))
        {//如果参数中存在 r=xxxx 并且脚本url为/index.php 则不执行url美化规则
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->enablePrettyUrl && !$this->hasRouteParam($request))
        {
    ...
    * @return string $url
    */
    protected function buildOriginalUrl($route, $params, $anchor = '')
    {
    ...
    public function createUrl($params)
    {
        ...
        //引入缓存机制 提升URL生成性能
        $urlCacheKey = __CLASS__ . __METHOD__ . json_encode([$route, $params]);
        $hash = md5(json_encode([$this->rules, $this->showScriptName, $this->enablePrettyUrl]));
        if(($data = $this->cache->get($urlCacheKey)) !== false && isset($data[1]) && $data[1] === $hash)
        {//缓存中有 直接返回
            return $data[0];
        }
            ...
            if ($url !== false) {
                if (strpos($url, '://') !== false) {
                    if ($baseUrl !== '' && ($pos = strpos($url, '/', 8)) !== false) {
                        $url = substr($url, 0, $pos) . $baseUrl . substr($url, $pos);
                    } else {
                        $url .= $baseUrl . $anchor;
                    }
                } else {
                ...
                $this->cache->set($urlCacheKey, [$url, $hash]);
                return $url;
            }

            $url = $this->buildOriginalUrl($route, $params, $anchor);
        } else {
            $url = $this->buildOriginalUrl($route, $params, $anchor);
        }
        $this->cache->set($urlCacheKey, [$url, $hash]);

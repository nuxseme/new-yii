 ...
 * @since 2.0
 */
class AppUrlRule extends Object implements UrlRuleInterface
{
    /**
    ...
    public function init()
    {
        ...
        $this->pattern = ltrim($this->pattern, '/');//只去除左边的'/'
        $this->route = trim($this->route, '/');

        if ($this->host !== null) {
            $this->host = rtrim($this->host, '/');
            $this->pattern = $this->host . '/' . $this->pattern;
        } elseif ($this->pattern === '') {
            $this->_template = '';
            $this->pattern = '#^$#u';
            ...
            $this->pattern = '/' . $this->pattern;//只在左边加'/'
        }
        ...
        $this->pattern = '#^' . ltrim(strtr($this->_template, $tr), '/') . '$#u';//不要去掉右边的'/'
    ...
    public function createUrl($manager, $route, $params)
    {
        ...
        $url = ltrim(strtr($this->_template, $tr), '/');//不要去掉右边的'/'
        if ($this->host !== null) {
            $pos = strpos($url, '/', 8);
            if ($pos !== false) {

	...
	*/
    ...
    { 
    	...
    	{ //购物车有商品，根据仓库ID,HashCode修改或增加对应的值
    		$oldQty = intval($oldShippingCartArray[$addCartWarehouseId][$addCartHashcode][0]['qty']);
    ...
    public function actionAjaxshowcart(){ 
    	...
    	{ 
	    	...
	    	$plistJson = str_replace(
	    								'listing', 
	    								'listingId',
	    								str_replace('qty', 'num', str_replace('}]', '}', str_replace('[{', '{', Json::encode($newPlist))))
	    							);
    	...
    	if($shippingCartInfo)
    	{
            foreach($shippingCartInfo as $index => $each) 
            {
                $each['url'] = $this->TTHelper->getProductUrl($each['url']);
                $each['imageUrl'] = $this->TTHelper->getThumbnailUrl('product', $each['imageUrl'], Yii::$app->params['cartImgWidth'], Yii::$app->params['cartImgHeight']);
                $shippingCartInfo[$index] = $each;
            }
    		return $this->resAjax(['ret' => 1, 'data' => ['count' => count($shippingCartInfo), 'result' => $shippingCartInfo]]); 
	    }
	    else
	    { 
	    	return $this->resAjax(['ret' => 0, 'data' => ['count' => 0]]);
